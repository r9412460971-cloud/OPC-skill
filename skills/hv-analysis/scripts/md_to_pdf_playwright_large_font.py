#!/usr/bin/env python3
"""
横纵分析法报告 Markdown → PDF 转换脚本 (Playwright 大字版)
在原版基础上整体放大一级字号，提升阅读舒适度。
"""
...
def md_to_pdf(md_path: str, pdf_path: str, title: str = None, author: str = "R9（资产配置研习社）"):
    ...
    with sync_playwright() as p:
        ...
        page.pdf(
            ...
            header_template=f'<div style="font-size:9px; color:#95a5a6; width:100%; text-align:center; padding:0 20mm; border-bottom:0.5pt solid #ecf0f1; padding-bottom:3mm;">{cover_title} | 横纵分析法深度研究报告</div>',
            footer_template='<div style="font-size:10px; color:#666; width:100%; text-align:center; padding:0 20mm;"><span class="pageNumber"></span> / <span class="totalPages"></span></div>'
        )
        browser.close()
...
def main():
    parser = argparse.ArgumentParser(description="横纵分析法报告 Markdown → PDF 转换 (Playwright大字版)")
