#!/usr/bin/env python3
"""
主动管理基金调仓有效性验证报告 Markdown → PDF 转换脚本 (Playwright 备选版)
...
def md_to_pdf(md_path: str, pdf_path: str, title: str = None, author: str = "R9（资产配置研习社）"):
    ...
    cover_title = h1_match.group(1) if h1_match else (title or "调仓有效性验证报告")
    # 移除正文中的第一个h1（用于封面）
    ...
    # 提取元信息（如存在）
    meta_match = re.search(r'<blockquote>\s*<p>(.*?)</p>\s*</blockquote>', html_body_for_content)
    ...
    <div class="cover">
        <h1>{cover_title}</h1>
        <div class="subtitle">主动管理基金调仓有效性验证报告</div>
        <div class="divider"></div>
        <div class="meta">
            作者：{author}<br>
            {f'元信息：{meta_info}' if meta_info else ''}
        </div>
    </div>
    {toc_html}
    ...
    with sync_playwright() as p:
        ...
        page.pdf(
            ...
            header_template=f'<div style="font-size:8px; color:#95a5a6; width:100%; text-align:center; padding:0 20mm; border-bottom:0.5pt solid #ecf0f1; padding-bottom:3mm;">{cover_title} | 主动管理基金调仓有效性验证报告</div>',
...
def main():
    parser = argparse.ArgumentParser(
        description="调仓有效性验证报告 Markdown → PDF 转换 (Playwright版)",
